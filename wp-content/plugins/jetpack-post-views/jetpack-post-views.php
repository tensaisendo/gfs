...
.light {
	...
	position: relative;
	top: 3px;
	box-shadow: 0 1px 2px #fff, 0 -1px 1px #666, inset 1px -2px 6px
		rgba(0, 0, 0, 0.5), inset -1px 1px 6px rgba(255, 255, 255, 0.8);
}
...
<div class="wrap">
	...
	<form method="post" action="options.php">
			...
			<tr valign="top">
				...
				<td><label for="jetpack-post-views-display-total-views"> <input
						name="jetpack_post_views_settings[display_total_views]"
						class="checkbox" type="checkbox"
						<?php checked( $this->settings['display_total_views'], 'on' ); ?>
						id="jetpack-post-views-display-total-views" /> <span><?php _e( 'Display total views for each post on the "All Posts" page.', 'jetpack-post-views' ); ?></span>
				</label></td>
			</tr>
			<tr valign="top">
				<th scope="row"><label for="jetpack-post-views-use-stats-get-csv"><?php _e( 'Stats_get_csv', 'jetpack-post-views' ); ?></th>
				<td><label for="jetpack-post-views-use-stats-get-csv"> <input
						name="jetpack_post_views_settings[use_stats_get_csv]"
						class="checkbox" type="checkbox"
						<?php checked( $this->settings['use_stats_get_csv'], 'on' ); ?>
						id="jetpack-post-views-use-stats-get-csv" /> <span><?php _e( 'Use the Jetpack function <code>stats_get_csv()</code> to update post views. Disable this option to use the Blog URI or Blog ID instead (for example, if you are using a different site\'s settings).', 'jetpack-post-views' ); ?></span>
				</label></td>
			</tr>
			<tr valign="top">
				<th scope="row"><label for="jetpack-post-views-use-blog-uri"><?php _e( 'Blog URI', 'jetpack-post-views' ); ?></th>
				<td><label for="jetpack-post-views-use-blog-uri"> <input
						name="jetpack_post_views_settings[use_blog_uri]" class="checkbox"
						type="checkbox"
						<?php checked( $this->settings['use_blog_uri'], 'on' ); ?>
						id="jetpack-post-views-use-blog-uri" /> <span><?php _e( 'Use the Blog URI and WordPress API Key to update post views. If you are not using the Jetpack function <code>stats_get_csv()</code>, disabling this option may help correctly update post views.', 'jetpack-post-views' ); ?></span>
				</label></td>
			</tr>
		</table>
		...
		<table class="form-table">
			<tr valign="top">
				...
				<td><input name="jetpack_post_views_settings[api_key]" type="text"
					id="jetpack-post-views-api-key"
					value="<?php echo esc_attr( $this->settings['api_key'] ); ?>"
					class="regular-text"
					placeholder="<?php _e("https://apikey.wordpress.com/", 'jetpack-post-views'); ?>" /></td>
			</tr>
			<tr valign="top">
				<th scope="row"><label for="jetpack-post-views-blog_uri"><?php _e( 'Blog URI', 'jetpack-post-views' ); ?></label></th>
				<td><input name="jetpack_post_views_settings[blog_uri]" type="text"
					id="jetpack-post-views-blog-uri"
					value="<?php echo esc_attr( $this->settings['blog_uri'] ); ?>"
		...
		<p><?php _e( 'Shows the status of connections to the Jetpack API. If at least one of the below connections shows green, the plugin should work properly.<br>Stats updated using the first connection available in the order that they are listed.', 'jetpack-post-views' ); ?></p>
            ...
            <input type="hidden"
			name="jetpack_post_views_settings[connect_blog_uri]"
			id="jetpack-post-views-connect-blog-id"
			value="<?php echo esc_attr( $this->settings['connect_blog_uri'] ); ?>" />
		<input type="hidden"
			name="jetpack_post_views_settings[connect_blog_id]"
			id="jetpack-post-views-connect-blog-id"
			value="<?php echo esc_attr( $this->settings['connect_blog_id'] ); ?>" />
		<table class="form-table">
			<fieldset>
				<tr valign="top">
					...
					<td>
						<div
							class="light <?php echo (function_exists('stats_get_csv') ? "green" : "red") ?>">
				...
				<tr valign="top">
					...
					<td>
						<div
							class="light <?php echo ($this->settings['connect_blog_uri'] ? "green" : "red") ?>">
							<div class="inner-light"></div>
						</div>
					</td>
				</tr>
				<tr valign="top">
					...
					<td>
						<div
							class="light <?php echo ($this->settings['connect_blog_id'] ? "green" : "red") ?>">
							<div class="inner-light"></div>
						</div>
					</td>
				</tr>
		...
		<a
			href="https://www.paypal.com/cgi-bin/webscr?cmd=_s-xclick&hosted_button_id=CPUDV9EYETJYJ"
			title="Donate to this plugin!"> <img
			src="https://www.paypal.com/en_US/i/btn/btn_donate_LG.gif" alt="" />
		</a>
		</p>
...
<div id="JPV-display-content" class="JPV-tab-content active">
	...
	<p>
		...
		<input class="widefat"
			...
			value="<?php echo $instance['title']; ?>" type="text" />
	</p>
	...
	<p>
		...
		<input id="<?php echo $this->get_field_id( 'num_posts' ); ?>"
			...
			value="<?php echo $instance['num_posts']; ?>" type="text" size="3" />
	</p>

	<p>
		<label for="<?php echo $this->get_field_id( 'days' ); ?>"><?php _e('Time interval:', 'jetpack-post-views'); ?></label>
		<select id="<?php echo $this->get_field_id( 'days' ); ?>"
			name="<?php echo $this->get_field_name( 'days' ); ?>">
			<option value="<?php echo $this->interval['Unlimited'] ?>"
				<?php echo ($instance['days'] == $this->interval['Unlimited'] ? 'selected' : '') ?>><?php _e('Unlimited', 'jetpack-post-views'); ?></option>
			<option value="<?php echo $this->interval['Day'] ?>"
				<?php echo ($instance['days'] == $this->interval['Day']       ? 'selected' : '') ?>><?php _e('Day',       'jetpack-post-views'); ?></option>
			<option value="<?php echo $this->interval['Week'] ?>"
				<?php echo ($instance['days'] == $this->interval['Week']      ? 'selected' : '') ?>><?php _e('Week',      'jetpack-post-views'); ?></option>
			<option value="<?php echo $this->interval['Month'] ?>"
				<?php echo ($instance['days'] == $this->interval['Month']     ? 'selected' : '') ?>><?php _e('Month',     'jetpack-post-views'); ?></option>
			<option value="<?php echo $this->interval['Year'] ?>"
				<?php echo ($instance['days'] == $this->interval['Year']      ? 'selected' : '') ?>><?php _e('Year',      'jetpack-post-views'); ?></option>
		</select>
	</p>

	<p>
		<input class="checkbox" type="checkbox"
			<?php checked( $instance['show_views'], 'on' ); ?>
			id="<?php echo $this->get_field_id( 'show_views' ); ?>"
			name="<?php echo $this->get_field_name( 'show_views' ); ?>" /> <label
			for="<?php echo $this->get_field_id( 'show_views' ); ?>"><?php _e('Display number of views?', 'jetpack-post-views'); ?></label>
	</p>
	...
	<p>
		...
		<select id="<?php echo $this->get_field_id( 'display_type' ); ?>"
			...
			<option value="title"
				<?php echo ($instance['display_type'] == 'title'           ? 'selected' : '') ?>><?php _e('Title',             'jetpack-post-views'); ?></option>
			<option value="thumbnail"
				<?php echo ($instance['display_type'] == 'thumbnail'       ? 'selected' : '') ?>><?php _e('Thumbnail',         'jetpack-post-views'); ?></option>
			<option value="thumbnail_title"
				<?php echo ($instance['display_type'] == 'thumbnail_title' ? 'selected' : '') ?>><?php _e('Thumbnail + Title', 'jetpack-post-views'); ?></option>
		</select>
	</p>
</div>
...
<div id="JPV-filter-content" class="JPV-tab-content">
	...
	<p>
		...
		<input class="widefat"
			id="<?php echo $this->get_field_id( 'exclude_posts' ); ?>"
			name="<?php echo $this->get_field_name( 'exclude_posts' ); ?>"
			value="<?php echo $instance['exclude_posts']; ?>" type="text"
			placeholder="<?php _e('Comma-separated list of post IDs', 'jetpack-post-views'); ?>" />
	</p>
	...
	<p>
                ...
                ?>
                            <input class="checkbox" type="checkbox"
			<?php checked( $instance['type_'.$key], 'on' ); ?>
			id="<?php echo $this->get_field_id( 'type_'.$key ); ?>"
			name="<?php echo $this->get_field_name( 'type_'.$key ); ?>" /> <label
			for="<?php echo $this->get_field_id( 'type_'.$key ); ?>"><?php echo $key ?></label>
		<br />
                <?php
                        }
                    }
	...
	<p>
                ...
                ?>
                        <input class="checkbox" type="checkbox"
			<?php checked( $instance['category_'.$value->slug], 'on' ); ?>
			id="<?php echo $this->get_field_id( 'category_'.$value->slug ); ?>"
			name="<?php echo $this->get_field_name( 'category_'.$value->slug ); ?>" />
		<label
			for="<?php echo $this->get_field_id( 'category_'.$value->slug ); ?>"><?php echo $value->name ?></label>
		<br />
                <?php
                    }
                ?>
